...
**First-person graffiti simulator. HTML5, WebGL, Three.js. Framework-free JavaScript.**

*It's not a shooting game. It's a tracking game: mark territory and leave your signature.*
...
[![Play Now](https://img.shields.io/badge/PLAY_NOW-FF9900?style=for-the-badge&logo=github&logoColor=black)](https://eduardofierroduque-sudo.github.io/WDL-MASTER-TAGGER-VERSION-METRO-/)
...
## The Collection -- 7 Scenarios Playable from the Browser

<div align="center">

| # | Scenario | Type | Style | Play |
|---|---|---|---|---|
| 1 | **Maestranza Metro/Tren** | Railway Workshops, Santiago | Free | [PLAY](https://eduardofierroduque-sudo.github.io/WDL-MASTER-TAGGER-VERSION-METRO-/juego%20graffiti%20METRO/) |
| 2 | **NYC Subway** | New York City Subway | Free | [PLAY](https://eduardofierroduque-sudo.github.io/WDL-MASTER-TAGGER-VERSION-METRO-/juego%20graffiti%20trenes%202/) |
| 3 | **BA Subte** | Buenos Aires Subway | Danger | [PLAY](https://eduardofierroduque-sudo.github.io/WDL-MASTER-TAGGER-VERSION-METRO-/juego%20graffiti%20trenes%203/) |
| 4 | **Container 1** | Industrial Shipyard | Free | [PLAY](https://eduardofierroduque-sudo.github.io/WDL-MASTER-TAGGER-VERSION-METRO-/juego%20grafiti%20BODEGA/) |
| 5 | **Bodega Fluor** | Dark Maze | Free | [PLAY](https://eduardofierroduque-sudo.github.io/WDL-MASTER-TAGGER-VERSION-METRO-/juego%20grafiti%20BODEGA%20FLUOR/) |
| 6 | **Abandoned Building** | Skyscraper, 5 Floors | Free | [PLAY](https://eduardofierroduque-sudo.github.io/WDL-MASTER-TAGGER-VERSION-METRO-/juego%20grafiti%20EDIFICIO/) |
| 7 | **Night Maze** | Walled Outdoor Maze | Free | [PLAY](https://eduardofierroduque-sudo.github.io/WDL-MASTER-TAGGER-VERSION-METRO-/juego%20grafiti%20LABERINTO/) |

</div>

> All maps include integrated radio with music produced by Eduardo Fierro, via YouTube IFrame API. Each map is a single HTML file that loads instantly. No downloads, no installations.
...
<p align="center">
  ...
  <br>
  <a href="https://youtu.be/KE8L0RL6YC8">WATCH TRAILER ON YOUTUBE</a>
</p>
...
El `Raycaster` is cast from the camera position toward the center of the screen, with a withfigurable range of 6 to 15 units depending on the map. If the ray hits a surface, a `DecalGeometry` is placed at the impact point, oriented according to the intersected face normal. Decal size is withtrolled with the valve (slider en UI).
...
Each drip has randomized properties generated at spawn: fall speed (`dripSpeed`, between 0.08 and 0.25 units per frame), initial length (`dripLength`, between 0.05 and 0.3), and a random lateral offset to prevent all drips from falling in a perfect line. The drip stretches progressively on the Y axis (`scale.y += dripSpeed * 0.1`) and displaces downward (`position.y -= dripSpeed`). Cuando `scale.y` exceeds a maximum or the drip leaves the visible range, it is removed from the `activeDrips`.
...
Specifically in Bodega Fluor, if the distance the mouse travels between frames is less than 0.05 units, `dripCounter` increments by 2 per frame (fast accumulation). If the mouse moves more, it only increments by 0.5.
...
### Moving Trains and AABB Collision

Exclusive to the BA Subte map. Two trains run on fixed side tracks (X = -100 y X = +100). Each train has 4 cars assembled with `BoxGeometry`, totaling approximately 300 units in length. They carry a `PointLight` headlight acting as a beawith (`color: #ffeecc, intensity: 3, distance: 400`), visible through the fog.

The movement logic is linear and deterministic: Train 1 moves in the +Z direction from Z = -550 until it disappears at Z > 650. Train 2 moves in the -Z direction from Z = +550 until Z < -650. Speed is 3.0 units per frame (at 60fps, roughly 180 virtual meters per sewithd). Upon disappearing, a respawn is scheduled with `setTimeout`: Train 1 every 120 sewithds exactly, Train 2 every 120 sewithds but offset (starts 75s after the first). The first train appears 15 sewithds after the game starts, the sewithd at 75 sewithds.
...
### Snapshot Mode -- Screenshot Capture
...
| Mapa | Densidad | Fog Color |
|---|---|---|
| Metro | 0.008 | #020202 |
| NYC Subway | 0.012 | #020202 |
| BA Subte | 0.008 | #020202 (changes to #330000 on alert) |
| Bodega | 0.004 | #0a0a0b |
| Bodega Fluor | 0.03 | #020202 |
| Edificio | 0.01 | #87CEEB (cielo) |
| Laberinto | 0.02 | #87CEEB (cielo) |

NYC has the densest fog (0.012) to accentuate the enclosed tunnel atmosphere. Bodega Fluor has 0.03, nearly opaque at 50 meters. Bodega has the lightest (0.004), allowing the warehouses to be seen at a distance. Edificio y Laberinto use sky-blue color as they are daytime maps.

### Per-Scenario Lighting
...
First-person movement is implemented without unog `PointerLockControls` from Three.js. The camera rotates directly based on `movementX` y `movementY` from the `mousemove`. Pitch (Y) is clamped between -PI/2 and PI/2.

WASD movement is relative to the camera orientation. A forward vector is calculated from `camera.rotation.y` usando `no/cos`, and a perpendicular right vector. Jumping uses `playerVelocity.y` which decreases by gravity each frame. A ground check keeps the player at Y = playerHeight.

Parameters by map:

| Mapa | Speed | Sprint | Gravity | Jump | Height |
|---|---|---|---|---|---|
...
- Movement: 0.45 caminando, 0.75 corriendo (Shift)
- Gravity: 0.012, salto: 0.32, altura jugador: 2.4
- Spray range: 15 units
- Valve: 0.1 a 1.4, step 0.05, default 0.4
- 533 lines of code

[PLAY](https://eduardofierroduque-sudo.github.io/WDL-MASTER-TAGGER-VERSION-METRO-/juego%20graffiti%20METRO/) | [itch.io](https://fierroduque.itch.io/wdl-master-tagger)
...
- Movement: 0.45 / 0.75 (sprint)
- Gravity: 0.012, salto: 0.32, altura: 2.4
- Spray range: 15, Valve: 0.1 a 5.0, default 1.0
- Goteo: DRIP_THRESHOLD 15, max 400 gotas activas
- 666 lines of code

[PLAY](https://eduardofierroduque-sudo.github.io/WDL-MASTER-TAGGER-VERSION-METRO-/juego%20graffiti%20trenes%202/) | [itch.io](https://fierroduque.itch.io/wdl-master-tagger-ny-subway)
...
[PLAY](https://eduardofierroduque-sudo.github.io/WDL-MASTER-TAGGER-VERSION-METRO-/juego%20graffiti%20trenes%203/) | [itch.io](https://fierroduque.itch.io/wdl-master-tagger-tren-3) | [Gameplay](https://www.youtube.com/watch?v=dHxB3NoJs1k)
...
- Movement: 0.28 (no sprint)
- Gravity: 0.009, salto: 0.24, altura: 2.5
- Spray range: 12, Valve: 0.05 a 0.8, default 0.25
- Goteo: DRIP_THRESHOLD 15, max 400 gotas, anti-withstante por mouse
- Shadows: PCFSoftShadowMap activado
- 504 lines of code

[PLAY](https://eduardofierroduque-sudo.github.io/WDL-MASTER-TAGGER-VERSION-METRO-/juego%20grafiti%20BODEGA/) | [itch.io](https://fierroduque.itch.io/wdl-master-tagger-withtainers-1)
...
Maze hardcoded as a 2D matrix, not procedural. Walls are `BoxGeometry` of 8 units, height 6. Collision via `Math.abs`, no `Box3`. Aggressive drip: if you don't move the mouse, the counter increases at double speed. Designed to get lost in the maze, paint in the dark, and let the walls fill with drips.

- Movement: 0.20 (el mas lento)
- Gravity: 0.008, salto: 0.18, altura: 2.0
- Spray range: 6
- Goteo: DRIP_THRESHOLD 20, max 300 gotas, acumulacion acelerada
- Iluminacion: solo AmbientLight rojiza
- 365 lines of code

[PLAY](https://eduardofierroduque-sudo.github.io/WDL-MASTER-TAGGER-VERSION-METRO-/juego%20grafiti%20BODEGA%20FLUOR/)
...
50 random skyscrapers around: position, height, and dimensions randomized. They form an enveloping skyline. Daylight: `AmbientLight` 0.6, `DirectionalLight` 0.9 sun. Blue sky and fog `#87CEEB`. Concrete gray walls `#999999`, dark gray floors `#222222`. No drip, no enemies.
...
- Movement: 0.25
- Gravity: 0.008, salto: 0.22, altura: 2.0
- Spray range: 6, Valve: 0.05 a 0.6, default 0.25
- 5 pisos with rampas alternadas, 50 rascacielos procedurales
- 486 lines of code

[PLAY](https://eduardofierroduque-sudo.github.io/WDL-MASTER-TAGGER-VERSION-METRO-/juego%20grafiti%20EDIFICIO/) | [itch.io](https://fierroduque.itch.io/wdl-master-tagger-edificio-abandonado)

---

### 7. Night Maze

Walled Outdoor Maze. Grilla 8x10 (mismo layout que un piso del Edificio), walls 6 high. Black static train at the center -- an obstacle that divides the maze. Stepped blocks of 3 heights for parkour.
...
- Movement: 0.18 (el mas lento del set)
- Gravity: 0.008, salto: 0.22, altura: 2.0
- Spray range: 6, Valve: 0.05 a 0.6, default 0.25
- Attached Paint a objetos (hit.object.add + worldToLocal)
- Tren estatico central + parkour + withtainers perimetrales
- 511 lines of code

[PLAY](https://eduardofierroduque-sudo.github.io/WDL-MASTER-TAGGER-VERSION-METRO-/juego%20grafiti%20LABERINTO/) | [itch.io](https://fierroduque.itch.io/wdl-master-taggerlaberinto-de-noche)

---

## Scenario Comparison

| Feature | Metro | NYC | BA Subte | Container | Fluor | Edificio | Laberinto |
|---|---|---|---|---|---|---|---|
| Lineas de codigo | 533 | 666 | 990 | 504 | 365 | 486 | 511 |
| DecalGeometry | No | Si | Si | No | No | No | No |
| Drip System | No | Si | Si | Si | Si | No | No |
| Moving Trains | No | No | Si | No | No | No | No |
| Game Over | No | No | Si | No | No | No | No |
| 20s Alert | No | No | Si | No | No | No | No |
| Sprint | Si | Si | Si | No | No | No | No |
| Shadows | No | No | No | Si | No | No | No |
| Multiple Floors | No | No | No | No | No | Si | No |
| Attached Paint | No | No | No | No | No | No | Si |
| Daylight | No | No | No | No | No | Si | Si |
| Dual Audio | No | No | Si | No | No | No | No |
| Speed mov | 0.45 | 0.45 | 0.45 | 0.28 | 0.20 | 0.25 | 0.18 |
...
| `SHIFT` | Run (only in Metro, NYC, BA Subte) |
| `ESPACIO` | Jump |
| `MOUSE` | Look around |
| `CLICK IZQUIERDO` | Spray paint |
| `P` | Screenshot (PNG) |
| `R` | Respawn |
| `ESC` | Release pointer |
...
- **Language:** JavaScript Vanilla, no TypeScript or transpilers
...
- **Controls:** Pointer Lock API + eventos de teclado/mouse nativos
...
- [x] Moving Trains with colision AABB (BA Subte)
- [x] Radio YouTube integrada with musica original
- [x] Snapshot mode (captura PNG)
- [x] Menu principal with acceso a todos los mapas
- [x] Dual Audio with fallback para ejecucion local
- [x] Shadows proyectadas (Container 1)
...
[![GitHub Pages](https://img.shields.io/badge/PLAY_NOW-FF9900?style=for-the-badge&logo=github&logoColor=black)](https://eduardofierroduque-sudo.github.io/WDL-MASTER-TAGGER-VERSION-METRO-/)
[![itch.io](https://img.shields.io/badge/itch.io-FA5C5C?style=for-the-badge&logo=itch.io&logoColor=white)](https://fierroduque.itch.io/)
[![YouTube](https://img.shields.io/badge/YouTube-FF0000?style=for-the-badge&logo=youtube&logoColor=white)](https://www.youtube.com/@WDLMASTERTAGGER)
[![Website](https://img.shields.io/badge/fierroduque.com-000000?style=for-the-badge&logo=safari&logoColor=white)](https://fierroduque.com)
...
| Video | Link |
|---|---|
| Official Trailer | [youtu.be/KE8L0RL6YC8](https://youtu.be/KE8L0RL6YC8) |
...
├── index.html                          # Main menu with access to the 7 maps
...
<p align="center">
  <em>WDL -- From the street to the code.</em><br>
  <strong>(c) 2025-2026 Eduardo Fierro -- fierroduque.com</strong><br><br>
  <sub>No generative AI was used in the creation of visual assets. Handmade, line by line.</sub>
</p>
