...
**第一人称涂鸦模拟器. HTML5, WebGL, Three.js. JavaScript sin frameworks.**

*这不是射击游戏. Es un juego de trackear: 占领领地留下你的签名.*
...
[![立即游玩](https://img.shields.io/badge/PLAY-FF9900?style=for-the-badge&logo=github&logoColor=black)](https://eduardofierroduque-sudo.github.io/WDL-MASTER-TAGGER-VERSION-METRO-/)
...
> 全地图集成YouTube IFrame API电台 integrada con musica producida por Eduardo Fierro, via YouTube IFrame API. 每张地图为单个HTML文件即时加载。无需下载安装。
...
En el resto de 地图一览 se usa `CircleGeometry` con `MeshBasicMaterial`, aplicando `renderOrder` incremental y `polygonOffset` con factor variable por capa. Es mas ligero pero no tiene la misma fidelidad de textura de aerosol.
...
Hay un limite de gotas activas (300 a 400 segun el mapa). Cuando se alcanza, las gotas mas antiguas se eliminan primero (FIFO). En 地图一览 Bodega y Bodega Fluor, el contador de goteo se resetea o se ralentiza si el jugador mueve el mouse, simulando que el aerosol se distribuye sobre una superficie mas amplia en lugar de acumularse en un punto.
...
Todos 地图一览 integran la YouTube IFrame API. Se crea un elemento iframe oculto que carga el reproductor de YouTube sin interfaz y comienza a reproducir una playlist con musica original producida por Eduardo Fierro.
...
Todos 地图一览 usan `FogExp2` de Three.js, que implementa niebla exponencial cuadratica: la visibilidad decae segun `exp(-density * distance^2)`. La densidad varia entre mapas:
...
地图一览 de tren comparten parametros agiles con sprint. Los demas son mas lentos, pensados para exploracion pausada.
...
Menu bilingue ES/EN con 操控, mision y advertencias. Audio dual (YouTube en servidor, HTML5 Audio en local). Manejo avanzado de eventos: prevencion de menu contextual, reseteo en mouseleave/blur, ignorancia del primer mousemove.
...
- **操控:** Pointer Lock API + eventos de teclado/mouse nativos
...
- [x] Menu principal con acceso a todos 地图一览
...
<p align="center">
  ...
  <sub>视觉资源未使用生成式AI en la creacion de assets visuales. 纯手工逐行打造.</sub>
</p>
